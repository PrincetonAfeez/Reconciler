...
# Import standard library modules for file handling, logging, and CSV operations
import csv
import logging
...
def menu() -> None:
    ...
    from reconciler_cli import menu as cli_menu

    cli_menu()


def _build_arg_parser():
    from reconciler_cli import _build_arg_parser as cli_build_arg_parser

    return cli_build_arg_parser()


def run_cli_args(args) -> int:
    """Run one reconciliation from parsed CLI args; return process exit code."""
    from reconciler_cli import run_cli_args as cli_run_cli_args

    return cli_run_cli_args(args)


def _fuzzy_threshold_from_percent(value: float) -> float:
    """Convert percent-like input to a clamped 0..1 threshold."""
    from reconciler_cli import _fuzzy_threshold_from_percent as cli_fuzzy_threshold_from_percent

    return cli_fuzzy_threshold_from_percent(value)


def main(argv: list[str] | None = None) -> None:
    """Entry point: CLI when arguments are present, otherwise interactive menu."""
    from reconciler_cli import main as cli_main
